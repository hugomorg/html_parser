defmodule HTMLParser.TreeBuilderTest do
  ...
  describe "build/1 builds tree from parsed open and close tags" do
    test "single tag" do
      assert {:ok, TreeBuilder.build(div: open_tag(), div: close_tag()) == HTMLNodeTree.new(:div)}
    end

    test "single tag with text" do
      assert {:ok,
              TreeBuilder.build([{:div, open_tag()}, "hi", div: close_tag()]) == %HTMLNodeTree{
                tag: :div,
                children: [
                  HTMLTextNode.new("hi")
                ]
    ...
    test "single empty tag" do
      assert {:ok, TreeBuilder.build(meta: open_tag()) == HTMLNodeTree.new(:meta)}
    end

    test "single tag with different child" do
      assert {:ok,
              TreeBuilder.build(div: open_tag(), p: open_tag(), p: close_tag(), div: close_tag()) ==
    ...
    test "sibling tags - different" do
      assert {:ok,
              TreeBuilder.build(div: open_tag(), div: close_tag(), p: open_tag(), p: close_tag()) ==
                [
    ...
    test "single tag with empty child" do
      assert {:ok,
              TreeBuilder.build(div: open_tag(), input: open_tag(), div: close_tag()) ==
                %HTMLNodeTree{tag: :div, children: [HTMLNodeTree.new(:input)]}}
    end

    test "sibling tags - same" do
      assert {:ok,
              TreeBuilder.build(p: open_tag(), p: close_tag(), p: open_tag(), p: close_tag()) == [
                HTMLNodeTree.new(:p),
                HTMLNodeTree.new(:p)
              ]}
    end
    ...
    test "sibling empty" do
      assert {:ok,
              TreeBuilder.build(p: open_tag(), p: close_tag(), input: open_tag()) == [
                HTMLNodeTree.new(:p),
